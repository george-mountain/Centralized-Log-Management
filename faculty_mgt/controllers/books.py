...
def list_all_books():
    with requests.Session() as sess:
        response = sess.get("http://localhost:8000/school-management/library/book/list")
...
def list_all_request():
    with requests.Session() as sess:
        response = sess.get(
            "http://localhost:8000/school-management/library/book/request/list",
        )
        return response.json()
...
def request_borrow_book(request: BookRequestReq):
    with requests.Session() as sess:
        response = sess.post(
            "http://localhost:8000/school-management/library/book/request",
...
def list_all_issuance():
    with requests.Session() as sess:
        response = sess.get(
            "http://localhost:8000/school-management/library/book/issuance/list"
        )
        return response.json()
...
def return_book(returning: BookReturnReq):
    with requests.Session() as sess:
        response = sess.post(
            "http://localhost:8000/school-management/library/book/issuance/return",
